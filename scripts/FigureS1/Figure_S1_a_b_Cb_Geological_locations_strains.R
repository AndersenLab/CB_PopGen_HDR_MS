...
group_list<- read.table("../../data/isotype_groups.tsv",header = TRUE)
nrow(group_list)
#2022
...
unique((indep_strain_info$isotype) %in% unique(group_list$isotype))
unique((unique(group_list$isotype) %in% indep_strain_info$isotype))


group_list_join<-group_list %>% 
  select(strain,isotype)

indep_strain_info<-indep_strain_info %>% 
  select(-isotype) %>% 
  left_join(group_list_join, by = "strain")
...
#  dplyr::filter(long > -152  & long < 154 & lat > -25 & lat < 58) %>%
#   dplyr::filter(!(long > 120  & long < 122 & lat > 21.7 & lat < 25.5))
...
plot_world <-ggplot2::ggplot()+ geom_map(data=world, map=world,
  ...
  geom_point(data = indep_strain_info_geo_output, aes(x= long, y=lat, color = geo), shape =16, size = 1, alpha = 1.0) +
...
geo_freq <- indep_strain_info_geo_output %>%
  dplyr::group_by(geo) %>%
...
plot_freq2 <- ggplot2::ggplot(geo_freq2) +
  ...
  geom_text(aes(x=1,y=group+0.5,label=geo),
            fontface = "bold", hjust=0, size = 1.8) +
  geom_text(aes(x=frequency+25,y=group,label=as.character(frequency)),size = 1.8) + 
  scale_fill_manual(values=geo.colours) +
...
saveRDS(p, file = "../../processed_data/assemble_figure_S1/strain_map.rds")
